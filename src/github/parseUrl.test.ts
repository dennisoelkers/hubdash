...
describe('parsePrUrl accepted forms', () => {
  const expected = { owner: 'Graylog2', repo: 'graylog2-server', number: 4821 };

  it('parses a plain PR URL', () => {
    expect(expectOk('https://github.com/Graylog2/graylog2-server/pull/4821')).toEqual(expected);
  });
  ...
  it('parses a URL with a trailing path segment', () => {
    expect(expectOk('https://github.com/Graylog2/graylog2-server/pull/4821/files')).toEqual(expected);
    expect(expectOk('https://github.com/Graylog2/graylog2-server/pull/4821/commits')).toEqual(expected);
  });

  it('parses a URL with a fragment', () => {
    expect(
      expectOk('https://github.com/Graylog2/graylog2-server/pull/4821#discussion_r123456'),
    ).toEqual(expected);
  });
  ...
  it('parses http as well as https', () => {
    expect(expectOk('http://github.com/Graylog2/graylog2-server/pull/4821')).toEqual(expected);
  });

  it('parses the owner/repo#number shorthand', () => {
    expect(expectOk('Graylog2/graylog2-server#4821')).toEqual(expected);
  });

  it('ignores surrounding whitespace and newlines', () => {
    expect(expectOk('  https://github.com/Graylog2/graylog2-server/pull/4821\n')).toEqual(expected);
  });

  it('preserves owner and repo casing as given', () => {
    // prKey does the case-folding; the parser must not silently rewrite input.
    expect(expectOk('https://github.com/GRAYLOG2/Server/pull/7').owner).toBe('GRAYLOG2');
  });
});
...
describe('parsePrUrl rejections', () => {
  ...
  it('rejects a non-GitHub host', () => {
    expect(expectError('https://gitlab.com/Graylog2/graylog2-server/pull/4821')).toMatch(
      /github\.com/i,
    );
  });

  it('rejects an issue URL', () => {
    expect(expectError('https://github.com/Graylog2/graylog2-server/issues/4821')).toMatch(
      /pull request/i,
    );
  });

  it('rejects a repo URL with no PR number', () => {
    expect(expectError('https://github.com/Graylog2/graylog2-server')).toMatch(/pull request/i);
    expect(expectError('https://github.com/Graylog2/graylog2-server/pull')).toMatch(/number/i);
  });

  it('rejects a non-numeric PR number', () => {
    expect(expectError('https://github.com/Graylog2/graylog2-server/pull/abc')).toMatch(/number/i);
  });

  it('rejects a zero or negative PR number', () => {
    expect(expectError('https://github.com/Graylog2/graylog2-server/pull/0')).toMatch(/number/i);
    expect(expectError('Graylog2/graylog2-server#0')).toMatch(/number/i);
  });
