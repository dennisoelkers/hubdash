...
export function App({ deps = {} }: { deps?: AppDeps } = {}) {
  ...
  const poll = useCallback(async () => {
    if (token === null || prs.length === 0) return;
  ...
  }, [token, prs, fetchImpl, nowMs]);

  // A rejected token will be rejected again; a hard rate limit needs the reset
  // to pass. Both stop the loop rather than hammering GitHub.
  const rateLimited =
  ...
  const { refresh, isPolling } = usePolling({ enabled, intervalMs: POLL_INTERVAL_MS, poll });
  ...
  const columns = useMemo(() => {
    ...
    const tracked = new Set(prs.map((pr) => `${pr.owner.toLowerCase()}/${pr.repo.toLowerCase()}#${pr.number}`));
...
Expected: every test passes. If the "stops polling on a rejected token" test sees two calls, check that `enabled` really goes false — the auth error must be set before the refresh click resolves.
